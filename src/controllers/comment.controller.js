...
export const createComment = async (req, res) => {
    try {
        ...
        else if (affectedRows === 1 && profileId !== 1) { return res.status(200).json({ message: `Comentario creado por el usuario numero: ${profileId}` }) }

        else {return res.status(500).json({ message: `Error Interno al crear el comentario` }) }
    } catch (error) {
