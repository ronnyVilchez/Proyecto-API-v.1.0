...
export const createPublication = async (req, res) => {
    try {
        ...
        else if (affectedRows === 1 && profileId !== 1) { return res.status(200).json({ message: `Publicacion creado por el usuario numero: ${profileId}` }) }

        else { res.status(500).json({ message: `Error Interno al crear la publicacion` }) }
